...
namespace UzMarket.ServiceLayer.MediatorServices.AddressServices.Commands
{
    ...
    public class DeleteAddressHandler : IRequestHandler<DeleteAddressCommand, string>
    {
        ...
        {
            var address = await _context.Addresses.FirstOrDefaultAsync(x => x.Id == request.Id && x.UserId == _service.UserId && x.StatusId != (int)StatusIdConst.DELETED, cancellationToken);
